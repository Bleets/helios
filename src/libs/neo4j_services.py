...
from py2neo.matching import NodeMatcher,ENDS_WITH,STARTS_WITH
    ...
    class AWS:
      class Associations:
        def route53_cloudfront():
          ...
          if len(r53_nodes):
            for node in r53_nodes:
              domain_name = node["name"][:-1]
              cloudfront_node = matcher.match("CLOUDFRONT", DomainName=STARTS_WITH(domain_name)).first()
              if cloudfront_node:
                relation_route53_cloudfront = Relationship(node,"CONNECT_TO_CLOUDFRONT",cloudfront_node)
                neo4j_tx.create(relation_route53_cloudfront)
